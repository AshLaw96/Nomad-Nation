    ...
    def clean_extra_amenity(self):
        ...
        if extra_amenity:
            if not self.instance.owner:
                raise forms.ValidationError("Caravan must have an owner.")
            if Amenity.objects.filter(name=extra_amenity, owner=self.instance.owner).exists():
                raise forms.ValidationError("This amenity already exists for your account.")
        return extra_amenity

    def clean_available_dates(self):
        available_dates = self.cleaned_data.get('available_dates', '[]')
        try:
            dates = json.loads(available_dates)
            if not isinstance(dates, list):
                raise forms.ValidationError("Invalid date format. Expected a list of objects.")
        except json.JSONDecodeError:
            raise forms.ValidationError("Invalid JSON format for available dates.")
        # Validate each date entry
        for date_entry in dates:
            if not isinstance(date_entry, dict):
                raise forms.ValidationError("Each date entry must be a dictionary.")
            if 'start_date' not in date_entry or 'end_date' not in date_entry:
                raise forms.ValidationError("Each date entry must contain 'start_date' and 'end_date'.")
            try:
                start_date = datetime.strptime(date_entry['start_date'], "%Y-%m-%d").date()
                end_date = datetime.strptime(date_entry['end_date'], "%Y-%m-%d").date()
            except ValueError:
