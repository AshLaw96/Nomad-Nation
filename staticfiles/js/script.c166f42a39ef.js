    ...
    if (calendarEl) {
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            select: function (info) {
                let dates = hiddenInputEl.value.trim() ? JSON.parse(hiddenInputEl.value) : [];
            ...
            eventDrop: function (info) {
                let dates = JSON.parse(hiddenInputEl.value);
        ...
        modal.addEventListener('shown.bs.modal', function () {
            ...
            if (modalCalendarEl && modalHiddenInputEl && !modalCalendarEl.classList.contains('initialised')) {
                const modalCalendar = new FullCalendar.Calendar(modalCalendarEl, {
                    ...
                    unselectAuto: false,
                    select: function (info) {
                        let dates = modalHiddenInputEl.value.trim() ? JSON.parse(modalHiddenInputEl.value) : [];
                    ...
                    // Handle event dragging (date change)
                    eventDrop: function (info) {
                        let dates = JSON.parse(modalHiddenInputEl.value);
                        let index = dates.findIndex(d => d.start_date === info.oldEvent.startStr);
